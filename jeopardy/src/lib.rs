...
impl Jeopardy {
    ...
    pub fn categories(&self, show_number: &u32) -> Option<&Vec<Record>> {
        self.by_show.get(show_number)
    }
...
mod tests {
    ...
    #[test]
    fn it_works() {
        let collection = Jeopardy::new().unwrap();
        let category = collection.random().unwrap();
        let _s = Jeopardy::fmt_category(&category);
    }
}
